...
const postFakeLogin = async data => {
  try {
    // const {data} = await axios.post('http://18.169.95.14:3000/register',values)
    const formData = new FormData()
    formData.append("email", data.email)
    formData.append("password", data.password)
    // const response = await fetch('http://18.169.95.14:3000/login', {
    ...
    if (responseData.error) {
      // data.toast.error(responseData.msg)
      localStorage.setItem("errorMsg", responseData.msg)
    } else {
      if (responseData.body) {
        // console.log("responseData 1", responseData.body)
        const storeData = {
          uid: responseData.body[0]._id,
          username: responseData.body[0].username,
          role: "admin",
          email: responseData.body[0].email,
        }
        // console.log("responseData 2", storeData)
        localStorage.setItem("authUser", JSON.stringify(storeData))
        localStorage.setItem("accessToken", responseData.body[0].token)
        // data.toast.success("Successfully Verified")
      }
    }
    // console.log("responseData", responseData)
  } catch (err) {
